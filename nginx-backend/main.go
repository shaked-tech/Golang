...
func main() {
	...
	log.Printf("defaultFormat: %s",defaultFormat)
	log.Printf("errFilesPath %s contains:",errFilesPath)
	
    files, err = OSReadDir(errFilesPath)
    if err != nil {
        panic(err)
    }
...
func errorHandler(path, defaultFormat string) func(http.ResponseWriter, *http.Request) {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		log.Printf("path: %s, code: %s, ext: %s",path, code, ext)
